#!/usr/bin/env python3
"""Cadre Pierre carnet n°2 + note footer dans i18n article 77 (comme #76)."""
import json
import re
from pathlib import Path

JSON_PATH = Path(__file__).resolve().parent.parent / "locales/articles/77-les-techniques-de-peche.json"
...
HIGHLIGHT = {
    "en": (
        '<div class="highlight">\n'
        "<p><strong>Notebook No. 2 — Jean-Claude —</strong> My friend Pierre shared with me the notebooks of stories "
    ...
    "es": (
        '<div class="highlight">\n'
        "<p><strong>Cuaderno n° 2 — Jean-Claude —</strong> Mi amigo Pierre me hizo partícipe de los cuadernos de relatos "
...
TRANSITION = {
    "en": "<p><em>Here is one of these notebooks, entrusted to me by Pierre.</em></p>",
    "es": "<p><em>Aquí está uno de esos cuadernos, confiado en mis manos por Pierre.</em></p>",
}
...
H1 = {
    "en": (
        '<h1>🎣 #77 — Fishing Techniques of the 11 First Nations of Quebec<br>'
        '<span style="font-size:0.65em;color:#d4a574;">Pierre le Fouineur\'s Notebooks — Notebook No. 2</span></h1>'
    ),
    "es": (
        '<h1>🎣 #77 — Técnicas de Pesca de las 11 Primeras Naciones de Quebec<br>'
        '<span style="font-size:0.65em;color:#d4a574;">Cuadernos de Pierre le Fouineur — Cuaderno n° 2</span></h1>'
    ),
}


def patch_lang(html: str, lang: str) -> str:
    html = re.sub(r"<h1>🎣 #77.*?</h1>", H1[lang], html, count=1, flags=re.S)
    html = re.sub(r'<div class="highlight">.*?</div>', HIGHLIGHT[lang], html, count=1, flags=re.S)
    if "pierre-portrait" not in html:
        html = html.replace(HIGHLIGHT[lang], HIGHLIGHT[lang] + PORTRAIT_EN if lang == "en" else HIGHLIGHT[lang] + PORTRAIT_ES, 1)
        if TRANSITION[lang] not in html:
            portrait_end = html.find("</div>", html.find("pierre-portrait")) + len("</div>")
            html = html[:portrait_end] + "\n\n" + TRANSITION[lang] + html[portrait_end:]
    html = re.sub(
...
def main():
    if not JSON_PATH.exists():
        raise SystemExit(f"Missing {JSON_PATH} — run build-article-77-i18n.py first")
    data = json.loads(JSON_PATH.read_text(encoding="utf-8"))
    data["en"]["title"] = "#77 — Fishing Techniques of the First Nations of Quebec"
    data["es"]["title"] = "#77 — Técnicas de Pesca de las Primeras Naciones de Quebec"
    for lang in ("en", "es"):
